...
function Block() {
  ...
  const showPublicKeySchnorr = async () => {
    try {
      setLoading(true);
      setError(undefined);
      const key = await backend.get_schnorr_public_key();
      setKey(key);
    } catch (err) {
      console.error(err);
      setError(String(err));
  ...
  const signMessage = async () => {
    try {
      setLoading2(true);
      setError(undefined);
      const signature = await backend.sign_message_with_schnorr(message);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center space-x-4">
        ...
        >
          {loading ? "Loading..." : "Get Schnorr public key"}
        </button>
      </div>
